...
/**
 * @file app_napi.cpp
 * @brief NAPI Interface Implementation for XComponent
 * 
 * This file implements the bridge between ArkTS (JavaScript) and native C++ code,
 * handling XComponent lifecycle events and touch/mouse interactions.
 */
...
/**
 * @brief Get or create AppNapi instance for given XComponent ID
 * @param id XComponent identifier string
 * @return Pointer to AppNapi instance, nullptr on allocation failure
 * @note Uses singleton pattern per XComponent ID
 */
AppNapi* AppNapi::GetInstance(std::string& id)
{
    auto it = instance_.find(id);
    if (it == instance_.end()) {
        // Create new instance
        AppNapi* instance = new (std::nothrow) AppNapi(id);
        if (instance == nullptr) {
            LOGE("Failed to allocate AppNapi instance for id: %{public}s", id.c_str());
            return nullptr;
        }
...
napi_value AppNapi::ToggleWireframe(napi_env env, napi_callback_info info)
{
    (void)env;
    (void)info;
    if (tetrahedron_ != nullptr) {
        tetrahedron_->ToggleWireframe();
        tetrahedron_->Update(tetrahedron_->GetAngleX(), tetrahedron_->GetAngleY());
    }
    napi_value ret;
    napi_get_null(env, &ret);
