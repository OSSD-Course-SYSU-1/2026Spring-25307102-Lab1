...
function makePC(targetId) {
  ...
  setupDC(targetId, ch);
  pc.createOffer().then(offer=>{pc.setLocalDescription(offer);send({type:"webrtc-offer",targetId,data:offer});});
  return pc;
}
...
async function handleAnswer(fromId, ans) { if(pcs[fromId]) await pcs[fromId].setRemoteDescription(new RTCSessionDescription(ans)); }
async function handleICE(fromId, cand) { if(pcs[fromId]&&cand) await pcs[fromId].addIceCandidate(new RTCIceCandidate(cand)); }
